...
cc$dayofmonth <- as.factor(cc$dayofmonth) # Change to factor format
...
cc_loyalty <- cc %>% # Create a new dataframe that shows matches with a max distance difference of 1
...
cc_calendar$dayofmonth <- as.factor(cc_calendar$dayofmonth)
...
```
#### 5.1.2 Kronos Mart's Txns Are One Day Apart

```{r kronos-mart, eval = FALSE}

Q5.1.2 <- data.frame(dayofmonth = c(6:19))
Q5.1.2$dayofmonth <- as.factor(Q5.1.2$dayofmonth)
Q5.1.2 <- Q5.1.2 %>%
  left_join(cc, by = "dayofmonth") %>%
  filter(cc, location == "Kronos Mart") %>%
  group_by(dayofmonth) %>%
...
#### 5.1.5 Some CC txns occur at the same time (everyday at noon)
...
#### 5.2.1 No vehicle movements on weekends before midday?

#### 5.2.2 Large Distance Gap Between Consecutive GPS Records?

#### 5.2.3 Weird Driving by Elsa Orilla?
...
#### 5.4.1 8 Elsa Orilla and 13 Brand Tempesta Are Seeing Each Other

#### 5.4.2 21 Hennie Osvaldo Has Two Homes
...
#### 5.5.1 Presence of Anonymous Locations
...
#### 5.5.8 Bertrand Ovan driving a circle

...
```
#### 5.1.2 Investigation Needed To Ascertain True Transaction Dates of Kronos Mart's Books, Perhaps Through Receipt Verification
...
Q5.1.7_label_cc <- data.frame("id" = 1:54,
                              "code" = as_factor(cc_loyalty_1$last4ccnum))
Q5.1.7_label_loyalty <- data.frame("id" = 55:108,
                                   "code" = cc_loyalty_1$loyaltynum)
  
Q5.1.7_label <- bind_rows(Q5.1.7_label_cc,
                      Q5.1.7_label_loyalty)
Q5.1.7_label <- subset(Q5.1.7_label, select = -1 )

Q5.1.7 <- ggparcoord(cc_loyalty_1,
                     columns = 1:2,
                     groupColumn = 4,
                     showPoints = TRUE,
                     alphaLines = 1) +
  geom_text(aes(label= Q5.1.7_label$code),
            size = 3,
            nudge_x = 0.05) +

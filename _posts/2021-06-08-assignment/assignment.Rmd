---
title: "ISSS608 Assignment: VAST Challenge 2021 (Mini-Challenge 2)"
description: |
...
pacman::p_load(ggplot2, tidyverse, readr, grid, plotly, writexl, qwraps2, 
               ...
               lubridate, fuzzyjoin, ggridges, GGally)
...
#### 5.1.1 Popular locations seem to be food and beverage related
...
The txn dates of Kronos Mart differ by exactly one day, comparing either side of the credit card and loyalty data. Investigation needed to ascertain true transaction dates of Kronos Mart's books, perhaps through receipt verification. We will compare the GPS points in section 5.2.2.
...
#### 5.1.4 Some txns at coffee chains occur only at selected hours in the credit card data
...
```
#### 5.1.5 Able To Match 1-to-1 Credit Card with Loyalty Cards, except in selected cases
...
```{r no-vehicles-on-weekends-before-midday}
...
#### 5.2.2 GPS Seems To Conform To Kronos Mart's Loyalty Txns
In section 5.1.2 earlier, we mentioned of the possibility of transational data tampering specifically relating to Kronos Mart. Here, using looking at the GPS data, single visits on 9th, 11th, 12th, 13th, 15th, 16th and three visits on 18th suggests that these GPS visits matches more to loyalty data than the credit card data.

```{r kronos-mart-2, fig.height=4}

Q5.2.2_gps <- tibble("dayofmonth" = c(6:19)) %>%
                  left_join(spots %>%
...
#### 5.2.7 Large Distance Gap Between Consecutive GPS Records?
...
```{r fuzzyjoin-innerjoin-stringdist-cc-spots, eval=FALSE}
...
#### 5.3.99 Provide your answer and corresponding images here.
...
Hennie seems to stay in two separate homes on different evenings:  
- Either with Lidelse and Birgitta  
- Or with Inga, Loreto and Isia  

```{r hennie-osvaldo-two-homes}
...
**Suspicious Activities Can Be In The Following Form:**  
...
2) Individuals frequenting unusual places at abnormal hours
...
#### 5.5.6 Kronos Mart Midnight (low priority)

#### 5.5.7 Suspicious Activity By Nils Calixto (low priority)

#### 5.5.8 Bertrand Ovan driving a circle near midnight on 11th Jan 2014 (low priority)

#### 5.5.9 President only started appearing late in the period (low priority)
...
```{r suspicious-home-monitoring-draft, eval = FALSE}
...
```{r levenshtein-distance-matching-cc-loyalty, eval = FALSE}
...
```{r extract-cc-loyalty-pairs, eval = FALSE}
...
```{r levenshtein-distance-matching-cc-GPS, eval = FALSE}
...
```{r extract-cc-spots-pairs, eval = FALSE}
...
```{r calendar-ggplot-one, eval = FALSE}
...
ABC
Calendar Heatmap (GPS Points per day) http://visualdata.wustl.edu/varepository/VAST%20Challenge%202014/challenges/MC2%20-%20Patterns%20of%20Life%20Analysis/entries/University%20of%20Buenos%20Aires%20-%20Tralice/
Parallel Coordinates Graph (Location, Date, Timestamp, Price, Name) https://rpubs.com/tskam/PCP
Gantt Chart
Explore Hippokampos RadViz Clock Average Location https://www.cs.umd.edu/hcil/varepository/VAST%20Challenge%202014/challenges/MC2%20-%20Patterns%20of%20Life%20Analysis/entries/Central%20South%20University/

...
Q5.1.2_cc <- cc %>%
  ...
  group_by(dayofmonth) %>%
    summarise(cc_data = sum(price), .groups = "drop") %>%
  ungroup()
...
Q5.1.5_period <- data.frame("period" = unique(cc[c('period')]))
...
```
#### 5.1.6 Show Spending Outliers (low priority)
...
#### 5.2.2 GPS Seems To Validate Kronos Mart's Loyalty Txns
...
#### 5.2.4 Difference in locations between cc and gps data (Kronos Mart and others)? (low priority)

#### 5.2.5 Difference in cc and gps logs for President? (low priority)

#### 5.2.6 Axel Calzas does not go to work? (low priority)

#### 5.2.7 Large Distance Gap Between Consecutive GPS Records? (low priority)
